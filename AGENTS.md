...
For Echo, Foxtrot, Golf, server, and Pi — see the `AGENTS.md` in each subdirectory (`tracker_echo/`, `tracker_foxtrot/`, `tracker_golf/`, `server/`, `pi-display/`).
...
|-------|------|-------------|
| Backend Specialist | `.Codex/agents/backend-specialist.md` | Railway proxy server, ESP32 firmware, API integrations |
| Technical Writer | `.Codex/agents/technical-writer.md` | README, PI_PROXY_SETUP.md, inline comments, documentation |
| UI/UX Designer | `.Codex/agents/ux-designer.md` | UI implementation, layout, CRT aesthetic, accessibility, mobile responsiveness, TFT display UI |

---

## Gemini CLI Workflow

Gemini CLI (`gemini`) is installed and configured with shell helpers in `~/.bashrc`. Use it to **explore and draft** (free, large context), then bring targeted requests to Codex to **execute and integrate**.
